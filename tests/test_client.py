...
def test_json_data_str_or_bytes_with_ensure_ascii(simple_data):
...
def test_json_data_str_or_bytes_with_ensure_ascii_false(simple_data):
    data, data_encoding = simple_data
...
def test_json_data_is_none(simple_data):
    data, data_encoding = simple_data
    ensure_ascii: bool = True
    data: dict | None = None
...
def test_prepare_url_list_splitting() -> None:
    ...
    name = re.sub(r"[A-Z]", prepare_url, "contact_managecontactslists")
    split = name.split("_")
